...
C Evaluate the shape functions at the boundary sampling points
            ...
            EXISPB=RELPRP(IPPOS-1+IGAUSB,IELIDN)  ! the IGAUSB-th GAUSB point natural coord
            WEIGPB=RELPRP(IPWEI-1+IGAUSB,IELIDN)  ! the IGAUSB-th GAUSB point weight
            CALL SHPFUN
     1(   DERIV      ,DUMMY      ,EXISPB     ,1          ,IELTYP     ,
     2    MDIME      ,SHAPE      )
C Calculate components of the equivalent nodal loads
            DO 114 IDOFN=1,NDOFN
              PGASH(IDOFN)=R0     ! [normal pressure,tangent pressure] in gausp by shapefun
              DGASH(IDOFN)=R0     ! [dx/ds dy/ds] in gausp by shapefun
